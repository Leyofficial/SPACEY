...
function TrackOrderWrapper() {
  ...
  useEffect(() => {
    axios
      .get(`https://spacey-server-two.vercel.app/processOrder/${orderId}`)
      .then((res) => {
    ...
    <div className={style.block}>
      <header className={style.header}>
        <div className={style.textBlock}>
          <h2 className={style.orderId}>#{orderId}</h2>
      ...
      <main className={style.main}>
        <div className={style.stepper}>
          <CustomizedSteppers
            steps={steps}
            activeStep={orderInfo.orderActivity.length / 2}
          />
        </div>
      </main>
      <section className={style.orderActivity}>
        ...
        <div className={style.activityItems}>
          {orderInfo.orderActivity.map((item, index) => {
            return (
              <OrderActivity key={index} text={item.text} date={item.date} />
            );
          })}
        </div>
      </section>
