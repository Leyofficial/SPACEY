    ...
    return (
        <div className={style.block}>
            <CustomSearch  placeholder={'Search for brand...'} callback={setValueInput}/>
                ...
                {!isFound ? NotFound() : <div className={style.items}> {foundArrays.length > 0  ? foundArrays?.map((item: ICategory, index: number) => <SmallDealItem key={index} item={item}></SmallDealItem>) : Skeleton()}</div>}
