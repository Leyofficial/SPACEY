...
const ProductInfo = ({ product }: IProductInfoProps) => {
  ...
  const decreaseProduct = () => {
    if (countAddProduct > 1) setCountAddProduct(countAddProduct - 1);
  };
  ...
  const addToWishItem = () => {
    useAddToWish(user._id, product._id)
      .then((res) => {
            ...
            <section className={style.priceWrapper}>
              <p className={style.price}>
                ...
                {Number(product?.product.price) -
                  Number(product?.product.price) *
                    (Number(product.product.percentageOfSale) / 100)}
                .00 <span>${product.product.price}.00</span>
              </p>
              <p className={style.discount}>
                {product?.product.percentageOfSale}%{" "}
                {/* {product.product.saleType.toUpperCase()} */}
              </p>
            </section>
          </header>
          <main>
            <section className={style.techWrapper}>
              <div className={style.colorBlock}>
                ...
                <div className={style.wrapperColor}>
                  {product?.product.images.restImages.map((item) =>
                    getColorElement(item.color.toUpperCase())
                  )}
                </div>
                <CustomSelect
                  width={300}
                  items={product.product.memory}
                  title={"Memory"}
                ></CustomSelect>
              </div>
              <div className={style.sizeWrapper}>
                <CustomSelect
                  width={300}
                  items={product.product.size}
                  title={"Size"}
                ></CustomSelect>
                <CustomSelect
                  width={300}
                  items={product.product.storage}
                  title={"Storage"}
                ></CustomSelect>
              </div>
            </section>
