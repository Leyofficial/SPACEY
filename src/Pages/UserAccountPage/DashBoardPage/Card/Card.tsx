...
function Card({ cardData }: ICard) {
  ...
  const deleteClick = () => {
    axios
      .patch(
        `https://spacey-server-two.vercel.app/auth/card/delete/${user._id}`,
        {
  ...
  useEffect(() => {
    ...
    axios
      .post(`https://spacey-server-two.vercel.app/auth/card/edit/${user._id}`, {
        data: {
  ...
  return (
    <div
      ...
      style={
        cardData.number.startsWith("4")
          ? {
              background:
                "radial-gradient(236.15% 138.52% at 0% 0%, #1B6392 0%, #124261 100%)",
              color: "white",
            }
          : {
    ...
    >
        ...
        <Button aria-describedby={id} type="button" onClick={handleClick}>
          <div className={style.actionCard}>
            <HiOutlineDotsHorizontal
              color={cardData.number.startsWith("4") ? "white" : "#151515"}
            />
          </div>
        </Button>
        ...
        <div className={style.cardNumberBlock}>
          {isEditing ? (
            ...
            <input
              ...
              placeholder={cardData.number}
            ></input>
          )}
          <button
            ref={buttonRef}
            data-clipboard-text={numberToCopy}
      ...
      {!isEditing ? (
        <div
          onClick={() => setFinish(!finishEditing)}
