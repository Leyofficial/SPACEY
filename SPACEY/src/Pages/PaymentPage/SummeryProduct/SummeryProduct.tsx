    ...
    useEffect(() => {
        ...
        products?.map((product) => {
            getProduct(product.idProduct)
                .then(res => changeTotalHandler(res.data.found.product.price * Number(product.count)))
        ...
        <>
            <div className={style.wrapper}>
                ...
                <div className={style.infoPayment}>
                    ...
                    <p><span>Discount</span>{totalPrice && Math.round(totalPrice * 0.10)}</p>
                    <p><span>Tax</span>${totalPrice && Math.round(totalPrice * 0.05)}</p>
                </div>
                <div className={style.totalWrapper}>
                    <p className={style.totalTitle}>Total</p>
                    <p className={style.totalPrice}>{totalPrice - (totalPrice * 0.10 - (totalPrice * 0.17))} $</p>
                </div>
            </div>
        </>
    );
