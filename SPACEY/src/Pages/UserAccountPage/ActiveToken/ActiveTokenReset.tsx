...
function ActiveTokenReset({ callback }: ICallbackAccount) {
  ...
  const [tokenToResetPassword, setResetToken] = useState<string | null | any>(
    null
  );
  useEffect(() => {
    ...
    const handleSuccess = () => {
      ...
      setTimeout(() => {
        callback(true);
        navigate(
          `/user-account/login/reset-password?id=${tokenToResetPassword.foundUser._id}`
        );
      }, 2500);
    };
    const handleFailure = () => {
    ...
    if (
      tokenToResetPassword?.foundUser?.randomNumberToUpdatePassword === token
    ) {
      handleSuccess();
    } else {
      handleFailure();
  ...
  useEffect(() => {
    axios
      .get(`https://spacey-server-two.vercel.app/auth/find/${token}`)
      .then((res) => {
        if (res.status === 200) {
