...
function Login({ callback }: ILogin) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://spacey-server-two.vercel.app/auth/google?email=${email}&googleToken=${sub}`
      )
      .then((res) => {
        ...
        successAction(
          res.data.foundUser.googleToken,
          navigate,
  ...
  const submit: SubmitHandler<MyForm> = (dataFormInputs) => {
    axios
      .get(
        `https://spacey-server-two.vercel.app/auth?email=${dataFormInputs.email}&password=${dataFormInputs.password}`
      )
      .then((response) => {
      ...
      <div className={style.block}>
        ...
        <form onSubmit={handleSubmit(submit, error)}>
          ...
          <div className={style.passwordInput}>
            ...
            />
            <NavLink
              to={"/user-account/login/forget-password"}
