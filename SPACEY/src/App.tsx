...
export function App() {
  ...
  useEffect(() => {
    if (!token) return;
    axios
      .get(`https://spacey-server-two.vercel.app/auth/token/${token}`)
      .then((res) => {
        ...
        <Route path={"/"} element={<Layout />}>
          ...
          >
            ...
            <Route path={"user-account"} element={<UserAccountProfile />}>
              ...
              />
              <Route
                path={"/user-account/dashboard/addCard"}
                element={<AddCard />}
              />
              <Route path={"/user-account/setting"} element={<UserSetting />} />
              <Route
                path={"/user-account/logout"}
                element={<Logout callback={setPermissionLogin} />}
              />
            </Route>
          ...
          >
            <Route path={"user-account"} element={<UserAccount />}>
              <Route
                path={"reset-password/active/:token"}
                element={<ActiveTokenReset callback={setPermissionReset} />}
              ></Route>
              <Route
                element={
                  <PrivateRoute
                    to={"/user-account/login"}
                    isAuth={getPermission}
                  />
                }
              >
                <Route
              ...
              <Route
                path={"login"}
                element={<Login callback={setPermissionLogin} />}
              />
              <Route
                path={"sign-up"}
                element={<SignUp callback={setPermissionLogin} />}
              />
              <Route
